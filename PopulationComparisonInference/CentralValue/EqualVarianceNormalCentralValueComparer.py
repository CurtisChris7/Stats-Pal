...
class EqualVarianceNormalCentralValueComparer(IPopulationCentralValueComparer):
    """Class for comparing the central values of two independent single dimensional, normally distributed populations with equal variances"""

    def __init__(self, sample1: list, sample2: list, tDist: ITDistribution = SciPyTDistribution()) -> None:
        """
        Description
        ----------
        Constructor for the EqualVarianceNormalCentralValueAnalyzer
    ...
    def getConfidenceInterval(self, confidenceLevel: float) -> tuple:
        ...
        width: float = self.tDist.getTPercentileValue(1-alpha, self.df) * sqrt((1/self.n1) + (1/self.n2)) * self.sp
    ...
    def getTestStatistic(self, testDelta: float) -> float:
        ...
        return (self.mean1 - self.mean2 - testDelta) / (sqrt((1/self.n1) + (1/self.n2)) * self.sp)
